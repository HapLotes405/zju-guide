...
/** Skeleton for accordion sections during loading */
function AccordionSkeleton({ count = 8 }: { count?: number }) {
  return (
      ...
      {Array.from({ length: count }).map((_, i) => (
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="ml-auto h-5 w-5 rounded bg-slate-200" />
          </div>
        </div>
      ))}
    </div>
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-md">
      ...
      >
        ...
        <ChevronDown
          className={cn(
            "ml-auto h-5 w-5 flex-shrink-0 text-slate-400 transition-transform duration-200",
            open && "rotate-180",
          )}
        />
      </button>
...
export default function CourseDetailPage() {
  ...
  if (isError || !course) {
    return (
      <main className="mx-auto max-w-3xl px-4 py-8">
        <button
          onClick={() => router.back()}
          className="group mb-8 inline-flex items-center gap-1 text-sm text-slate-500 hover:text-indigo-600 transition-colors"
        >
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-8">
      ...
      <button
        onClick={() => router.back()}
        className="group mb-6 inline-flex items-center gap-1 text-sm text-slate-500 hover:text-indigo-600 transition-colors"
      >
        <ArrowLeft className="h-4 w-4" />
        返回
      </button>
      ...
      <div className="mb-8">
        <div className="mb-3 flex flex-wrap items-center gap-2">
          <span className="rounded-md bg-indigo-100 px-2.5 py-0.5 text-xs font-mono font-semibold text-indigo-700 tracking-wide">
            {course.code}
          </span>
          {isCompulsory ? (
      ...
      <div className="space-y-3">
        ...
        >
          <div className="grid gap-y-2.5 rounded-lg bg-slate-50 p-4">
            <InfoRow label="课号" value={course.code} />
        ...
        {/* 2. 为什么学 */}
        <AccordionSection id="why" icon={HelpCircle} title="为什么学">
          {/* Prerequisites */}
          <div className="mb-4">
        ...
        {/* 3. 课前预习 */}
        <AccordionSection id="preview" icon={BookMarked} title="课前预习">
            ...
            <div>
              ...
              <ul className="space-y-1.5">
                {MOCK_PREVIEW_POINTS.map((p, i) => (
                  <li
                    key={i}
                    className="flex items-center gap-2 rounded-md bg-indigo-50 px-3 py-2 text-sm text-indigo-800"
                  >
              ...
              <div className="space-y-1.5">
                {MOCK_PREVIEW_MATERIALS.map((m, i) => (
                  <a
                    ...
                    className="flex items-center gap-2 rounded-md px-3 py-2 text-sm text-indigo-600 transition hover:bg-indigo-50"
                  >
        ...
        {/* 4. 课中跟课 */}
        <AccordionSection id="during" icon={PenTool} title="课中跟课">
          <div className="space-y-4">
            {/* Weekly rhythm */}
            <div>
              <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-slate-400">
              ...
              <div className="space-y-1.5">
                {MOCK_WEEKLY_RHYTHM.map((w, i) => (
                  ...
                  >
                    <span className="whitespace-nowrap rounded bg-indigo-100 px-2 py-0.5 text-xs font-medium text-indigo-700">
                      {w.week}
                    </span>
        ...
        {/* 5. 期末复习 */}
        <AccordionSection id="final" icon={Target} title="期末复习">
          <div className="space-y-4">
            {/* Review route */}
            <div>
              <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-slate-400">
              ...
              <div className="space-y-2">
                {MOCK_REVIEW_ROUTE.map((step, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-3 rounded-md bg-indigo-50 px-4 py-2.5"
                  >
        ...
        {/* 6. 老师评价 */}
        <AccordionSection id="teachers" icon={Users} title="老师评价">
          <div className="space-y-3">
        ...
        {/* 7. 资源区 */}
        <AccordionSection id="resources" icon={FileText} title="资源区">
          <ResourceSection courseCode={course.code} courseName={course.name} />
        </AccordionSection>

        {/* 8. 图谱区 */}
        <AccordionSection id="graph" icon={GitFork} title="图谱区">
          <div className="space-y-2">
            {/* Current course (root) */}
            <div className="rounded-lg border-2 border-indigo-300 bg-indigo-50 p-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-indigo-600">{course.name}</span>
                <span className="ml-auto rounded-full bg-indigo-200 px-2 py-0.5 text-xs font-medium text-indigo-700">
                  当前
                </span>
              </div>
            </div>
              ...
              <div className="ml-4 border-l-2 border-dashed border-slate-200 pl-4">
                ...
                {course.dependents.map((d) => (
                  <Link
                    key={d.code}
                    href={`/course/${d.code}`}
                    className="mb-1.5 flex items-center gap-2 rounded-md border border-purple-200 bg-purple-50/50 px-3 py-2 transition hover:bg-purple-100"
                  >
                    <span className="text-sm font-medium text-purple-700">
                      {d.code}
                    </span>
                    <span className="text-xs text-purple-600">{d.name}</span>
                    {d.reason && (
  ...
  return (
    <div className="space-y-3">
      ...
      <a
        ...
        className="flex items-center justify-between rounded-lg border border-indigo-200 bg-indigo-50 px-4 py-4 transition hover:bg-indigo-100">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-indigo-600 text-white">
            <ExternalLink className="h-5 w-5" />
          </div>
          <div>
            <p className="text-sm font-medium text-indigo-800">在 CC98 搜索「{courseName}」</p>
            <p className="text-xs text-indigo-500">查看论坛讨论、真题回忆、经验分享</p>
          </div>
        </div>
        <ChevronDown className="h-5 w-5 -rotate-90 text-indigo-400" />
      </a>
      ...
      {!isLoading && !isError && resources.length === 0 && (
        <div className="rounded-lg border border-dashed border-slate-200 px-4 py-6 text-center">
          ...
          <p className="text-sm text-slate-400">暂无已审核资源，去
            <a href="/contribute" className="mx-1 text-indigo-500 hover:underline">投稿</a>
          贡献第一个</p>
        </div>
      )}
